...
const MyTests = () => {
    ...
    const handleJoinTest = () => {
        if (testCode.trim() !== '') {
            navigate(`/test-start/${testCode.trim()}`);
        } else {
            setError('Wpisz kod testu.');
            setTimeout(() => setError(''), 3000);
        }
    };
        ...
        <div className={styles.appContainer}>
            ...
            <div className={styles.mainContent}>
                ...
                <div className={styles.content}>
                    ...
                    <div className={styles.joinTestContainer}>
                        <div className={styles.joinTestTitle}>
                            <h3>Dołącz do testu</h3>
                            <div>
                                <input
                                    type="text"
                                    value={testCode}
                                    onChange={(e) => setTestCode(e.target.value)}
                                    placeholder="Wpisz kod testu"
                                />
                                <button onClick={handleJoinTest} className={styles.button}>
                                    Dołącz
                                </button>
                            </div>
                        </div>
                        ...
                        <div className={styles.testList}>
                            {tests.map((test) => (
                                <div key={test.code} className={styles.testCard}>
                                    ...
                                    <p onClick={() => handleOpenSet(test.Set.id)}>
                                        <strong>Zestaw:</strong> {test.Set.name}</p>
